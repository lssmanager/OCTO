...
describe('invokeRuntimeHttp', () => {
  ...
  it('throws so BullMQ can retry when runtime returns 500', async () => {
    ...
    await expect(invokeRuntimeHttp('http://runtime-worker:8000/api/v1/execute', 'secret', payload)).rejects.toBeInstanceOf(
  ...
  it('throws so BullMQ can retry on network failure', async () => {
    ...
    await expect(invokeRuntimeHttp('http://bad-runtime:8000/api/v1/execute', 'secret', payload)).rejects.toMatchObject({
