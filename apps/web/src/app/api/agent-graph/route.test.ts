...
function request(method: 'GET' | 'POST', init: { cookie?: string; origin?: string; body?: unknown } = {}) {
...
describe('F1 Agent Graph route authentication', () => {
  it('rejects unauthenticated graph reads without forwarding the server console token', async () => {
    ...
    await expect(res.json()).resolves.toEqual({ error: 'F1 Agent Graph console reads require an authenticated octo_console_token session cookie.' });
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it('forwards authenticated graph reads with only the caller session cookie token', async () => {
    process.env['OCTO_WEB_CONSOLE_TOKEN'] = 'SERVER_SIDE_CONSOLE_TOKEN';
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify([{ id: 'node-1' }]), { status: 200, headers: { 'content-type': 'application/json' } }));
    globalThis.fetch = fetchMock as unknown as typeof fetch;
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ headers: expect.objectContaining({ authorization: 'Bearer CALLER_SESSION_TOKEN' }) })
    );
  ...
  it('rejects unauthenticated writes even when server-token writes are enabled', async () => {
    ...
    const res = await POST(request('POST', { body: { action: 'patchNode', nodeId: 'node-1', body: { name: 'Updated' } } }));

    expect(res.status).toBe(401);
    await expect(res.json()).resolves.toEqual({ error: 'F1 Agent Graph console writes require an authenticated octo_console_token session cookie.' });
    expect(fetchMock).not.toHaveBeenCalled();
  });
  ...
  it('rejects cross-origin authenticated writes before forwarding to the API', async () => {
    ...
    const res = await POST(request('POST', { cookie: 'CALLER_SESSION_TOKEN', origin: 'https://evil.example.test', body: { action: 'patchNode', nodeId: 'node-1', body: { name: 'Updated' } } }));

    expect(res.status).toBe(403);
    await expect(res.json()).resolves.toEqual({ error: 'F1 Agent Graph console writes require a same-origin request.' });
    expect(fetchMock).not.toHaveBeenCalled();
  });
});
