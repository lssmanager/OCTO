...
async def run_f1_execution(execution_id:str, tenant_id:str, trace_id:str|None=None, mode:str='normal')->dict:
    ...
    try:
      async with conn.transaction():
        row=await conn.fetchrow("SELECT id, status AS state, version, input_json, context_snapshot_json FROM executions WHERE id=$1 AND tenant_id=$2",execution_id,tenant_id)
        if not row:
        ...
        if mode == 'reclaim':
          ...
          if row['state'] == 'reclaimable':
            await conn.execute("UPDATE executions SET state='running', status='running', updated_at=now() WHERE id=$1 AND tenant_id=$2 AND status='reclaimable'", execution_id, tenant_id)
        if row['state'] not in ('dispatched','reclaimable','running'):
          return {'status':'skipped','reason':'not_dispatched'}
        version=row['version']
        if row['state'] == 'dispatched':
          upd=await conn.execute("UPDATE executions SET state='running', status='running', version=version+1, started_at=now(), updated_at=now() WHERE id=$1 AND tenant_id=$2 AND status='dispatched' AND version=$3",execution_id,tenant_id,version)
          if upd!='UPDATE 1':
        ...
        await conn.execute("UPDATE executions SET state='completed', status='completed', version=version+1, result=$4::jsonb, output_json=$4::jsonb, completed_at=now(), updated_at=now(), last_checkpoint_id=$3 WHERE id=$1 AND tenant_id=$2 AND status='running'",execution_id,tenant_id,cp1,json.dumps({'output':output}))
