...
def test_disabled_and_needs_review() -> None:
    ...
    with pytest.raises(ToolDisabledError):
        r.resolve(t.name)
    t2 = http_request_definition(); t2.name = "http_request_2"; t2.status = ToolStatus.NEEDS_REVIEW
    r.register(t2)
...
def test_name_and_llm_format() -> None:
    r = ToolRegistry()
    t = http_request_definition(); t.name = "http://bad"
    with pytest.raises(ToolNameInvalidError):
        r.register(t)
    t2 = http_request_definition(); t2.name = "good_name"
    r.register(t2)
