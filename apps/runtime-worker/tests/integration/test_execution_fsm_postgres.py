...
async def _seed_execution(conn, execution_id: str, tenant_id: str, state: str = "QUEUED", version: int = 0) -> None:
    await conn.execute(
        ...
        INSERT INTO executions (
          id, tenant_id, agent_id, agent_version_id, state, version,
          attempt_count, reclaim_count, budget_snapshot_json,
          context_snapshot_json, created_by, created_at, updated_at
        ) VALUES ($1,$2,'agent-1','agent-v1',$3,$4,0,0,'{}'::jsonb,'{}'::jsonb,'test',now(),now())
...
async def _run_case() -> None:
    ...
    try:
        ...
        async with pool.acquire() as conn:
            await _seed_execution(conn, "f1-terminal", "tenant-a", state="SUCCEEDED", version=3)
        with pytest.raises(InvalidTransitionError):
            await fsm.transition("f1-terminal", "tenant-a", "SUCCEEDED", "RUNNING", 3)
    finally:
        await pool.close()
