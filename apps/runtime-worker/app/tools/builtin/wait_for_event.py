...
def wait_for_event_definition() -> ToolDefinition:
    return ToolDefinition(
        name="wait_for_event", kind=ToolKind.BUILTIN_ASYNC, description="Wait for an external event",
        ...
        side_effect_level=SideEffectLevel.NONE, approval_policy=ApprovalPolicy.POLICY_BASED,
    )


def execute_wait_for_event(args: dict) -> dict:
    _ = args
    return {"event_received": False, "payload": None}
