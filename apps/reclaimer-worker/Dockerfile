...
RUN pnpm turbo build --filter=@octo/reclaimer-worker
RUN pnpm --filter @octo/reclaimer-worker deploy --prod --legacy /prod/reclaimer-worker
...
ENV BUILD_VERSION=${BUILD_VERSION} \
    ...
    NODE_ENV=production
COPY --from=builder --chown=octo:octo /prod/reclaimer-worker ./
USER octo
EXPOSE 3011
