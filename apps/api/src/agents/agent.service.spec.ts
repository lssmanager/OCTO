...
describe('AgentService', () => {
  ...
  it('filters graph results to requested hierarchy scopes', async () => {
    const now = new Date();
    const nodes = [
      { id: 'agency-1', tenantId: 't1', level: 'agency', name: 'Agency 1', slug: 'agency-1', parentId: null, activationState: 'active', capabilities: [], createdAt: now, updatedAt: now },
      { id: 'dept-1', tenantId: 't1', level: 'department', name: 'Dept 1', slug: 'dept-1', parentId: 'agency-1', activationState: 'active', capabilities: [], createdAt: now, updatedAt: now },
      { id: 'workspace-1', tenantId: 't1', level: 'workspace', name: 'Workspace 1', slug: 'workspace-1', parentId: 'dept-1', activationState: 'active', capabilities: [], createdAt: now, updatedAt: now },
      { id: 'agent-node-1', tenantId: 't1', level: 'agent', name: 'Agent Node 1', slug: 'agent-node-1', parentId: 'workspace-1', activationState: 'active', capabilities: [], createdAt: now, updatedAt: now },
      { id: 'workspace-2', tenantId: 't1', level: 'workspace', name: 'Workspace 2', slug: 'workspace-2', parentId: 'dept-1', activationState: 'active', capabilities: [], createdAt: now, updatedAt: now },
      { id: 'agent-node-2', tenantId: 't1', level: 'agent', name: 'Agent Node 2', slug: 'agent-node-2', parentId: 'workspace-2', activationState: 'active', capabilities: [], createdAt: now, updatedAt: now },
    ];
    const agents = [
    ...
    expect(flat.map((node: any) => node.id)).toEqual(['workspace-1', 'agent-node-1']);
