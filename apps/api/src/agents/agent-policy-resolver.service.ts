...
export class AgentPolicyResolverService {
  ...
  async resolveEffectivePolicies(tenantId: string, agentId: string): Promise<EffectiveAgentPolicySnapshot> {
    ...
    const workspaceId = String(local.workspaceId ?? '');
    ...
    const primaryModel = String((local.modelPolicy as any)?.primaryModel ?? (base as any)?.modelPolicy?.primaryModel ?? '');
    const fallbackModels = unique([
      ...(((base as any)?.modelPolicy?.fallbackModels ?? []) as string[]),
      ...(((local.modelPolicy as any)?.fallbackModels ?? []) as string[]),
    ]).filter((m) => m !== primaryModel);

    const allow = unique([...( ((base as any)?.toolPolicy?.allow ?? []) as string[]), ...((((local as any).toolPolicy?.allow ?? []) as string[])]);
    const deny = unique([...( ((base as any)?.toolPolicy?.deny ?? []) as string[]), ...((((local as any).toolPolicy?.deny ?? []) as string[])]);
