...
export class HierarchyAccessGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (agencyId) {
      if (!user.agency_ids || user.agency_ids.length === 0) {
        throw new ForbiddenException({ code: 'AGENCY_SCOPE_MISSING' });
      }
      if (!user.agency_ids.includes(agencyId)) {
        throw new ForbiddenException({ code: 'AGENCY_SCOPE_DENIED' });
      }
    }
    if (workspaceId) {
      if (!user.workspace_ids || user.workspace_ids.length === 0) {
        throw new ForbiddenException({ code: 'WORKSPACE_SCOPE_MISSING' });
      }
      if (!user.workspace_ids.includes(workspaceId)) {
