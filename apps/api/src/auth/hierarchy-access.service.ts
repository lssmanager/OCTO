...
type HierarchyScope = { agencyId?: string; workspaceId?: string };

@Injectable()
export class HierarchyAccessService {
  private assertPrincipalScope(principal: AuthPrincipal, scope: HierarchyScope) {
    if (scope.agencyId && principal.agencyIds?.length && !principal.agencyIds.includes(scope.agencyId)) {
      throw new ForbiddenException({ code: 'AGENCY_SCOPE_DENIED' });
    }
    if (scope.workspaceId && principal.workspaceIds?.length && !principal.workspaceIds.includes(scope.workspaceId)) {
  ...
  private async resolveHierarchyNodeScope(tx: any, tenantId: string, nodeId: string) {
    const node = (
      await tx.select().from(hierarchyNodes).where(and(eq(hierarchyNodes.tenantId, tenantId), eq(hierarchyNodes.id, nodeId))).limit(1)
    )[0] ?? null;
  ...
  async assertCanAccessHierarchyNode(principal: AuthPrincipal, nodeId: string) {
    const scope = await withTenantTx(principal.tenantId, async (tx) => this.resolveHierarchyNodeScope(tx, principal.tenantId, nodeId));
