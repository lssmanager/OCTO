...
`docs/reports/f1-close-report.md` is generated by close mode. It includes UTC start/end time, commit SHA, branch, gate command, local tool environment, CI/deploy metadata, compose project, build phase/version/commit/time, verified public URLs, result per required check, skip policy and final decision.
