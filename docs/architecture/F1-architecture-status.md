...
- `scripts/f1-agent-graph-smoke.sh` validates persisted `Agency → Department → Workspace → Agent` creation, node detail, patch, activation/archive toggles, valid reparent, invalid hierarchy/self-parent/cycle/missing-parent paths, cross-tenant/nonexistent-node errors and selected Agent deletion against `/api/v1/agents/*`.
