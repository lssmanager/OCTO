...
- Reads use server-side `OCTO_WEB_CONSOLE_TOKEN`.
...
- `scripts/f1-agent-graph-smoke.sh` validates persisted `Agency → Department → Workspace → Agent` creation plus missing-credential, invalid-hierarchy and cross-tenant/nonexistent-node errors.
